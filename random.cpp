...
int main() {
  srand((unsigned int)time(
      nullptr)); // 初始化随季节种子，且只需要执行一次，以避免每次运行结果相同
  double num;
  ...
  default_random_engine
      int_engine; // 应只初始化一次，防止出现重复，及不要出现在函数中，可加static写在main函数外
  // 生成原始伪随机数序列（相当于“随机源”），是生成随机数的基础引擎
  uniform_int_distribution<unsigned int> udistribution(
      100000, 999999); // 应只初始化一次，防止出现重复
